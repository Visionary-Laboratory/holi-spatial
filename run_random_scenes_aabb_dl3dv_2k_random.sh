...
# 随机抽样 N 个 DL3DV 场景，先渲染 bbox 图片，再合成为视频。
#
# 示例：
#   bash run_random_scenes_aabb_dl3dv_random.sh -n 5
#   bash run_random_scenes_aabb_dl3dv_random.sh -n 10 --seed 42
#   bash run_random_scenes_aabb_dl3dv_random.sh -n 3 --json-dir /path/to/jsons --output-dir ./bbox_overlay_test_outputs_aabb_dl3dv

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

NUM_SCENES=""
JSON_DIR="${SCRIPT_DIR}/output_DL3DV_new/2K"
DATA_ROOT="${SCRIPT_DIR}/DL3DV/2K"
OUTPUT_DIR="${SCRIPT_DIR}/bbox_overlay_test_outputs_aabb_dl3dv_2k_gyy"
VIDEO_DIR="${OUTPUT_DIR}/videos"
MODEL_PATH="pgsr_DL3DV_all/2K"
FPS=6
SEED=""
...
print_help() {
  cat <<'EOF'
用法:
  bash run_random_scenes_aabb_dl3dv_random.sh -n <场景数量> [可选参数]

可选参数:
  -n, --num-scenes <int>   随机抽取的场景数（必填）
      --json-dir <path>    场景 JSON 目录（默认: output_DL3DV_new/2K）
      --data-root <path>   数据根目录（默认: DL3DV/2K）
      --output-dir <path>  渲染图片输出目录（默认: ./bbox_overlay_test_outputs_aabb_dl3dv_gyy）
      --video-dir <path>   视频输出目录（默认: <output-dir>/videos）
      --model-path <path>  3DGS 模型目录（默认: pgsr_DL3DV_all/2K）
      --fps <int>          输出视频帧率（默认: 6）
...
python json2bbox_images_depth_scannet.py output_scannetv2/scene0029_00.json   --data-root scannetv2/scans   --output-dir ./bbox_overlay_scene0029_00_outputs_aabb    --render wire --alpha 0.32  --no-include-desc --no-frustum-filter   --no-show-labels  -m pgsr_result_scannetv2/ --gpu-edge-visibility   --depth-eps 0.06   --depth-eps-scale 0.012   --edge-sample-density 4.0   --downscale 1   --thickness 3 --exclude-labels tile,tiles


python imgseq_to_video.py \
  --input_dir bbox_overlay_scene0029_00_outputs_aabb \
  --output bbox_overlay_scene0029_00_outputs_aabb/videos/out_scene0029_00.mp4 \
  --fps 20