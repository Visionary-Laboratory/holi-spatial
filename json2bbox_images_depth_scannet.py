...
def is_valid_pose(c2w: np.ndarray, K: np.ndarray) -> bool:
    """检查 pose 和 intrinsic 是否有效，避免奇异矩阵导致渲染失败。"""
    if c2w.shape != (4, 4) or np.any(np.isnan(c2w)) or np.any(np.isinf(c2w)):
        return False
    R = c2w[:3, :3]
    # 旋转矩阵应满足 det ≈ 1 且列向量近似单位长
    det = np.linalg.det(R)
    if not (0.9 < abs(det) < 1.1):
        return False
    for j in range(3):
        n = np.linalg.norm(R[:, j])
        if n < 0.9 or n > 1.1:
            return False
    if K is not None and (np.any(np.isnan(K)) or np.any(np.isinf(K)) or float(K[0, 0]) <= 0 or float(K[1, 1]) <= 0):
        return False
    return True
...
def load_transforms(scene_dir: Path) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray], str, Dict[str, Tuple[int, int]], Dict[str, str]]:
    ...
    Returns:
      ...
      scene_type     = "scannetppv2" or "dl3dv" or "scannet"
      size_map[stem] = (w,h)
      path_map[stem] = image_path
    """
    # ScanNet v2 style: color/, intrinsic/, pose/
    color_dir = scene_dir / "color"
    intrinsic_dir = scene_dir / "intrinsic"
    pose_dir = scene_dir / "pose"
    if color_dir.exists() and intrinsic_dir.exists() and pose_dir.exists():
        scene_type = "scannet"
        ...
        for ext in ("*.jpg", "*.jpeg", "*.png"):
            for img_path in sorted(color_dir.glob(ext)):
                stem = img_path.stem
        ...
        logging.info("Loaded ScanNet cameras: %d", len(intr_map))
...
def render_depths_with_gaussians(
    ...
    skipped_invalid_pose: List[str] = []
    with torch.no_grad():
        for stem in tqdm(stems_to_render, desc="Rendering depths"):
            c2w = c2w_map[stem]
            K = intr_map[stem]
            if not is_valid_pose(c2w, K):
                skipped_invalid_pose.append(stem)
                continue
            ...
            try:
                ...
                skipped_invalid_pose.append(stem)
                logging.debug("相机 %s 深度渲染异常: %s", stem, e)
    if skipped_invalid_pose:
        logging.warning("深度渲染跳过或失败 %d 个相机: %s", len(skipped_invalid_pose), skipped_invalid_pose[:5])
        if len(skipped_invalid_pose) > 5:
            logging.warning("  ... 等共 %d 个（可能为无效 pose 或奇异矩阵）", len(skipped_invalid_pose))
    return depth_map
    ...
    for i in range(3):
        if w[order[i]] < eps:
            ...
            if i == 0:
                ...
                fallback = np.cross(v[:, 0], np.array([0.0, 1.0, 0.0], dtype=np.float32))
                n = np.linalg.norm(fallback)
                fallback = fallback / n if n > eps else np.array([0.0, 1.0, 0.0], dtype=np.float32)
            else:
                fallback = np.cross(v[:, 0], v[:, 1])
                n = np.linalg.norm(fallback)
                fallback = fallback / n if n > eps else np.array([0.0, 0.0, 1.0], dtype=np.float32)
            #  Gram-Schmidt 正交化
            for j in range(i):
                fallback = fallback - np.dot(fallback, v[:, j]) * v[:, j]
            n = np.linalg.norm(fallback)
            v[:, i] = (fallback / n) if n > eps else fallback
    # make right-handed
...
def point_inside_box(point_w: np.ndarray, corners_w: np.ndarray, margin: float = 1e-4) -> bool:
    ...
    try:
        ...
        if np.any(np.isnan(axes)) or np.any(np.isinf(axes)):
            return False
        local = (point_w - c) @ axes
        coords = (corners_w - c) @ axes
        mins = coords.min(axis=0) - margin
        maxs = coords.max(axis=0) + margin
        return bool(np.all(np.isfinite(local)) and np.all(local >= mins) and np.all(local <= maxs))
    except Exception:
        return False
...
def _center_depth_visible(
    ...
    """label 在通过深度测试时显示；无 scene_depth 时无法测试，为兼容性默认显示 label"""
    x, y = np.round(center_uv).astype(int)
    if x < 0 or x >= img_w or y < 0 or y >= img_h:
        return False
    if scene_depth is None:
        return True  # 无深度数据时无法做深度测试，默认显示 label（避免深度渲染失败时全部不显示）
...
def draw_boxes_on_image(
        ...
        for zc, box, corners_w, corners_c, camera_inside in prepared:
            ...
            if show_labels:
                ...
                if zc2[0] > _NEAR_PLANE:
                    ...
                    roi = img_bgr[y0 : y1 + 1, x0 : x1 + 1].astype(np.float32)
                    box_col = np.array(col, dtype=np.float32)
                    blended = roi * (1 - 0.6) + box_col * 0.6
                    img_bgr[y0 : y1 + 1, x0 : x1 + 1] = np.clip(blended, 0, 255).astype(np.uint8)
                    text_col = (255, 255, 255) if (col[0] + col[1] + col[2]) < 384 else (0, 0, 0)
                    cv2.putText(img_bgr, text, (x0 + 3, y0 + th + 2),
                                cv2.FONT_HERSHEY_SIMPLEX, font_scale, text_col,
    ...
    if render == "solid" and show_labels:
        for zc, box, corners_w, corners_c, camera_inside in prepared:
            ...
            text_col = (255, 255, 255) if (col[0] + col[1] + col[2]) < 384 else (0, 0, 0)
            cv2.putText(img_bgr, text, (x0 + 3, y0 + th + 2),
                        cv2.FONT_HERSHEY_SIMPLEX, font_scale, text_col, 1, cv2.LINE_AA)
...
def process_once(
    ...
    save_legend: bool = False,  # 是否保存 label 图例（默认不保存，需传 --save-legend）
) -> None:
    ...
    # 生成 legend：仅包含过滤后剩余的 label，被 exclude 的不显示（需传 --save-legend 才保存）
    if save_legend:
...
def main():
    ...
    parser.add_argument(
        "--data-root",
        type=Path,
        default=Path("/mnt/shared-storage-user/intern7shared/liuyifei/scannetv2/scans"),
    ...
    parser.add_argument("--save-legend", action="store_true", help="保存 label 图例到 _legend.png（不传则不保存）")
