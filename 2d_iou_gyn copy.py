...
def get_mask_from_sam(
    ...
    # mask_input
    mask_tensor = torch.from_numpy((coarse_mask > 0).astype(np.float32))[None, None]
    mask_tensor = mask_tensor.to(sam3_processor.device)
    prompt.append_masks(mask_tensor)
    ...
    # point prompts（正=1，负=0）
    all_pts = pos_pts + neg_pts
    # if all_pts:
    #     pt_labels = [1] * len(pos_pts) + [0] * len(neg_pts)
    #     pts_tensor = torch.tensor(all_pts, dtype=torch.float32, device=sam3_processor.device).unsqueeze(1)
    #     lbl_tensor = torch.tensor(pt_labels, dtype=torch.long, device=sam3_processor.device).unsqueeze(1)
    #     pt_mask = torch.zeros((1, len(all_pts)), dtype=torch.bool, device=sam3_processor.device)
    #     prompt.append_points(pts_tensor, lbl_tensor, pt_mask)
...
def get_mask_from_sam3_tracker(
    ...
    # 采样点（你原来的采样函数可以继续用）
    pos_pts = _sample_points_from_mask(coarse_mask, max_positive_points)
    neg_pts = _sample_negative_points(coarse_mask, max_negative_points)
...
def process_scene(
    ...
    for image_name, label_masks in tqdm(gt_masks.items(), desc="处理图像"):
        ...
        for label, gt_mask in label_masks.items():
            ...
            coarse_iou = compute_2d_iou(pred_mask_expanded, gt_mask_resized)
            ...
            coarse_results.append({
                ...
                "pred_mask_area": int(np.sum(pred_mask_expanded > 0)),
                "gt_mask_area": int(np.sum(gt_mask > 0))
            })
            ...
            if image_path:
                ...
                try:
                    ...
                    Image.fromarray(pred_mask_expanded).save(coarse_mask_path, "PNG", compress_level=1)
                except Exception as e:
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("gyn_test_images/2d_iou_outputs_gyn_cmp_sam3"), help="输出目录")
