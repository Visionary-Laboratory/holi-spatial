...
def log_rerun_scene(
    ...
    rr.init(f"3d_bounding_gs/{scene}", spawn=False)
    ...
    if bboxes:
        ...
        for box in bboxes:
            ...
            xyz_min = corners.min(axis=0)
            xyz_max = corners.max(axis=0)
            centers.append((xyz_min + xyz_max) * 0.5)
            half_sizes.append((xyz_max - xyz_min) * 0.5)
            labels.append(box["label"])
            colors_box.append(label_color(box["label"])[:3])
        if centers:
            rr.log(
                "bboxes",
                rr.Boxes3D(
                    ...
                    colors=np.array(colors_box, dtype=np.uint8),
                    labels=labels,
                ),
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--rerun",
        action="store_true",
        help="启用 rerun 0.24.1 可视化（点云与 3D bbox）",
    )
    parser.add_argument(
        "--rerun-addr",
        type=str,
