...
DEFAULT_OUTPUT_DIR = Path("/home/liuyifei/code/posevlm/sam_masks_debug_evaluation_kuixuan")
    ...
    if isinstance(data, list):
        ...
        for entry in data:
            ...
            for item in items:
                ...
                if not img_name or not label or label == "__background__":
                    continue
                if img_name not in per_image:
                    per_image[img_name] = []
                if label not in per_image[img_name]:
                    per_image[img_name].append(label)
