...
def process_scene(
    ...
    for label, label_items in label_groups.items():
        ...
        # 1. 置信度过滤（不进行 VLM 召回）
        qualified_instances = []
        for inst in tqdm(instances, desc=f"Filtering {label} instances"):
            max_score = max((mask_score_map.get(img) or 0.0) for img in inst["images"])
            # 只保留 mask 分数 >= 0.9 的实例
            if max_score >= 0.9:
                qualified_instances.append(inst)
            else:
                logging.debug("过滤低分实例 (label: %s, highest_score: %.2f)", label, max_score)

        final_instances = qualified_instances 
        logging.info("标签 %s 置信度过滤后剩余 %d 个实例（已移除 VLM 召回）", label, len(qualified_instances))
