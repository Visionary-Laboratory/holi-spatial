...
def align_obb_to_up(
    ...
    for idx_x in other:
        idx_y = other[0] if other[1] == idx_x else other[1]

        v = axes[idx_x]
        proj = v - float(np.dot(v, z)) * z
        if float(np.linalg.norm(proj)) < 1e-8:
            continue
        x = _normalize(proj)
        if float(np.dot(x, v)) < 0.0:
            x = -x
        ...
        score = (
            abs(float(np.dot(R_new[:, 2], axes[idx_up])))
            + abs(float(np.dot(R_new[:, 0], axes[idx_x])))
            + abs(float(np.dot(R_new[:, 1], axes[idx_y])))
        )

        if extent_mode == "keep":
            ext_new = np.array([extents_full[idx_x], extents_full[idx_y], extents_full[idx_up]], dtype=np.float64)
        else:
    ...
    if best_R is None or best_ext is None:
        # Degenerate fallback: pick any x in the plane
        world_x = np.array([1.0, 0.0, 0.0], dtype=np.float64)
        x = world_x - float(np.dot(world_x, z)) * z
        if float(np.linalg.norm(x)) < 1e-8:
            world_y = np.array([0.0, 1.0, 0.0], dtype=np.float64)
            x = world_y - float(np.dot(world_y, z)) * z
        x = _normalize(x)
        y = _normalize(np.cross(z, x))
        best_R = _orthonormalize(np.column_stack([x, y, z]))
        if extent_mode == "keep":
            best_ext = extents_full.copy()
        else:
            local_new = (best_R.T @ X.T).T
            half_new = np.max(np.abs(local_new), axis=0)
            best_ext = 2.0 * half_new

    return best_R, best_ext
