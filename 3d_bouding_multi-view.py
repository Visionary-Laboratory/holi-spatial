...
def project_points_overlap(
    points: np.ndarray,
    K: np.ndarray,
    c2w: np.ndarray,
    mask: np.ndarray,
) -> int:
    """将 3D 点投影到 mask，返回与 mask 重叠的像素数。"""
    if points.shape[0] == 0:
        return 0
...
def process_scene(
    ...
    # 按标签存储实例列表，每个元素: {"points":[np.ndarray...], "images": set[str]}
    ...
    for item in items:
        ...
        inst_list = label_instances[label]
        assigned = False
        for inst in inst_list:
            pts_inst = np.concatenate(inst["points"], axis=0)
            if pts_inst.shape[0] > 50000:
                idx = np.random.choice(pts_inst.shape[0], 50000, replace=False)
                pts_inst = pts_inst[idx]
            overlap_cnt = project_points_overlap(pts_inst, intr_map[stem], c2w_map[stem], mask)
            if overlap_cnt >= overlap_min_pixels:
                inst["points"].append(points)
                inst["images"].add(str(mask_path))
                assigned = True
                break

        if not assigned:
            inst_list.append({"points": [points], "images": {str(mask_path)}})

    for label, inst_list in label_instances.items():
        logging.info("标签 %s 合并得到 %d 个实例", label, len(inst_list))
        for inst in inst_list:
    ...
    box_image_path = output_dir / f"{scene}_box_image.json"
    with box_image_path.open("w", encoding="utf-8") as f:
        json.dump(box_image, f, indent=2, ensure_ascii=False)
    logging.info("实例来源图像已保存: %s", box_image_path)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="前一实例投影到当前 mask 至少重叠像素数，判定为同一实例",
    )
    parser.add_argument(
        "--bbox-labels",
        type=str,
