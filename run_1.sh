...
OUTPUT_ROOT="output"
PROCESSED_ROOT="/home/liuyifei/code/posevlm/output_3d_bounding_evaluation_kuixuan"
...
# 从 recover_labels 目录读取场景名称
readarray -t SCENES < <(
    python - <<'PY'
from pathlib import Path

recover_labels_dir = Path("mask_index_outputs")
scenes = []
if recover_labels_dir.exists():
    for json_file in sorted(recover_labels_dir.glob("*.json")):
...
# 初始化状态记录文件
STATUS_JSON="3d_bounding_scannet_evaluation.json"
...
process_scene() {
    ...
    # 只运行 3d_bounding_instance_gs_rerun.py
