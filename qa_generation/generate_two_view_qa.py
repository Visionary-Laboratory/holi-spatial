...
def load_transforms_scannetv2(scene_id: str, data_root: Path) -> Tuple[Dict[str, float], List[FrameItem], Path]:
    """Load intrinsics/extrinsics and frame list for a ScanNetV2 scene.
    Scene structure: data_root/scene_id/ with color, intrinsic, pose folders.
    E.g. scannetv2/scans/scene0000_00/color, .../intrinsic, .../pose
    """
    scene_root = data_root / scene_id
    ...
    if not intrinsic_dir.exists() or not pose_dir.exists() or not images_dir.exists():
        raise FileNotFoundError(
            f"ScanNetV2 场景结构不完整: {scene_root}，需要 color、intrinsic、pose 文件夹"
        )
    ...
    if intrinsic_mat.shape[0] < 3 or intrinsic_mat.shape[1] < 3:
        raise ValueError(
            f"ScanNetV2 内参矩阵形状异常: {intrinsic_path}, shape={intrinsic_mat.shape}"
        )
    K = intrinsic_mat[:3, :3]

    pose_files = sorted([f for f in pose_dir.iterdir() if f.suffix == ".txt"])
    frames: List[FrameItem] = []
    width = height = None
    ...
    if not frames or width is None or height is None:
        raise RuntimeError(f"ScanNetV2 未找到有效帧: {scene_id}")
...
def main():
    ...
    def iter_scene_ids() -> List[str]:
        ...
        for p in sorted(root.iterdir()):
            if not p.is_dir():
                continue
            if (p / "dslr" / "nerfstudio" / "transforms_undistorted.json").exists() or (p / "dense" / "rgb").exists() or (p / "color").exists():
                out.append(p.name)
        return out
    ...
    for idx, scene_id in enumerate(all_scene_ids):
        ...
        scannetv2_scene = args.data_root / scene_id
        scannetv2_has = (
            (scannetv2_scene / "intrinsic").exists()
            and (scannetv2_scene / "pose").exists()
            and (scannetv2_scene / "color").exists()
        )
        use_rub_view = False
        if scannet_meta.exists():
            ...
            intrinsics, frames, images_dir = load_transforms_scannetv2(scene_id, args.data_root)
        else:
            print(f"跳过场景 {scene_id}: 未找到 scannetppv2、DL3DV 或 ScanNetV2 的相机参数文件")
            continue
        ...
        if bbox_items:
            ...
            if len(bbox_indices) < 2:
                ...
                if valid_mask_obj.sum() < 2:
                    ...
                    pairs = sample_pairs(covis_obj, valid_mask_obj, args.covis_threshold, args.num, rng, frames=frames_obj, min_translation_m=0.2)
                    ...
                    pairs = sample_pairs(covis_obj, valid_mask_obj, args.covis_threshold, args.num, rng, frames=frames_obj, bbox_items=bbox_items, at_least_3_objs=True)
        ...
        entries = entries_translation + entries_rotation + entries_object_depth + entries_object_dir + entries_inter_object + entries_object_height_or_length + entries_cross_dir + entries_proxy_dir + entries_relpos
