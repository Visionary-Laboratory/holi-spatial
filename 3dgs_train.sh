...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT_DIR/3dgs_outputs}"
LOG_FILE="${LOG_FILE:-$ROOT_DIR/train_3dgs_progress.json}"
MAX_JOBS_PER_GPU="${MAX_JOBS_PER_GPU:-3}"
DONE_PLY_REL="point_cloud/iteration_30000/point_cloud.ply"

usage() {
  cat <<'EOF'
Train PGSR/3DGS models for ScanNet v2, ScanNet++, or DL3DV scenes.

Required:
  SCENE_ROOT=/path/to/scenes       Directory containing scene folders.

Optional:
  OUTPUT_ROOT=3dgs_outputs         Directory for trained 3DGS models.
  LOG_FILE=train_3dgs_progress.json
  MAX_JOBS_PER_GPU=3               Parallel training jobs per GPU.

Examples:
  SCENE_ROOT=scannetv2/scans OUTPUT_ROOT=pgsr_scannetv2_all bash 3dgs_train.sh
  SCENE_ROOT=scannetppv2/data OUTPUT_ROOT=pgsr_scannetppv2_all bash 3dgs_train.sh
EOF
}
...
if [ -z "${SCENE_ROOT:-}" ]; then
  usage >&2
  echo "Error: SCENE_ROOT is required." >&2
  exit 2
fi

if [ ! -d "$SCENE_ROOT" ]; then
  echo "Error: SCENE_ROOT does not exist or is not a directory: $SCENE_ROOT" >&2
  exit 2
fi
...
for name in sorted(os.listdir(scene_root)):
    ...
    # 检测 ScanNet v2 格式：pose/ 和 color/
    is_scannet = os.path.isdir(os.path.join(d, "pose")) and os.path.isdir(
        os.path.join(d, "color")
    )
    # 检测 ScanNet++ 格式：dslr/nerfstudio/transforms_undistorted.json
    is_scannetpp = os.path.isfile(
        os.path.join(d, "dslr", "nerfstudio", "transforms_undistorted.json")
    ) and os.path.isdir(os.path.join(d, "dslr", "resized_undistorted_images"))
    if is_dl3dv or is_scannet or is_scannetpp:
        keep.append(name)
    else:
        skip_missing += 1
...
run_scene() {
  ...
  (
    ...
    local source_path="$scene_dir"
    local image_arg=()
    if [[ -f "$scene_dir/dslr/nerfstudio/transforms_undistorted.json" ]]; then
      source_path="$scene_dir/dslr/nerfstudio"
      image_arg=(-i "$scene_dir/dslr/resized_undistorted_images")
    elif [[ -d "$scene_dir/dense/cam" && -d "$scene_dir/dense/rgb" ]]; then
      image_arg=(-i "rgb")
    fi
