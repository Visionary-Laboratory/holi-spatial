...
def load_transforms(scene_dir: Path, image_names: Optional[set[str]] = None) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray], str, Dict[str, Tuple[int, int]], Dict[str, str], Dict[str, np.ndarray]]:
    ...
    # dl3dv
        ...
        size_map: Dict[str, Tuple[int, int]] = {}
        path_map: Dict[str, str] = {}
        mask_map: Dict[str, np.ndarray] = {}  # 对于 DL3DV，不再使用 mask，保持接口兼容
        ...
        for idx, cam_file in tqdm(enumerate(cam_files),total=len(cam_files),):
            ...
            # 对于 DL3DV，不再加载 mask（保持接口兼容，但不使用）
            # mask_map[stem] = np.ones((img_h, img_w), dtype=bool)  # 不再需要
...
def load_da3_depths(
    ...
    if image_names is not None:
        ...
        stems_to_load = [s for s in target_stems if s in c2w_map]
    else:
        stems_to_load = list(c2w_map.keys())
    
    logging.info("准备加载 da3 深度，数量: %d", len(stems_to_load))
    
    for stem in tqdm(stems_to_load, desc="Loading da3 depths"):
        # 加载深度图
        # 对于 DL3DV：格式为 frame_XXXXX.npy
        # 对于 ScanNet++ v2：格式为 stem.npy（例如 DSCXXXXX.npy）
        depth_path = depth_dir / f"{stem}.npy"
        if not depth_path.exists():
            logging.warning("深度文件不存在: %s", depth_path)
            continue
        ...
        if img_path.exists():
            if img_path.suffix in ['.png', '.jpg', '.jpeg']:
                color = np.array(Image.open(img_path).convert("RGB"), dtype=np.uint8)
    ...
    logging.info("da3 深度加载完成，数量: %d", len(depth_map))
...
def render_depths_with_gaussians(
    ...
    注意：对于 DL3DV 场景，应该使用 load_da3_depths 而不是这个函数。
    """
