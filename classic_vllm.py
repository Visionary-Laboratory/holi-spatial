...
def resolve_image_subdir(data_root: Path) -> str:
    data_root_str = str(data_root).lower()
    if "scannetv2" in data_root_str:
        return "color"
    if "scannetppv2" in data_root_str:
        return "dslr/resized_undistorted_images"
    if "dl3dv" in data_root_str:
        return "dense/rgb"
...
def discover_scenes(data_root: Path, scene_names: Sequence[str] | None) -> List[Path]:
    ...
    image_path = resolve_image_subdir(data_root)
    ...
    return [
        {
            ...
            "content": [
                ...
                "text": (
                    ...
                    "- Use the base class noun only: do NOT add quantity, shape, material, state, or descriptive modifiers "
                    "(e.g., use 'paper' not 'stack of papers'/'a stack of paper'; use 'chair' not 'office chair'; use 'box' not 'cardboard box').\n\n"
                    "No skipping rule:\n"
                    "- You MUST label every distinct object you can see.\n"
                    "- If you are unsure of an exact subtype, choose the closest reasonable, still-concrete supercategory (e.g., 'chair' instead of a specific chair type; "
                    ...
                    "- If any object in the current image matches a previous label in meaning (including synonyms, singular/plural, or minor wording differences), "
...
def save_scene_result(
    ...
    categories_set = set(categories) | {"floor", "ceiling", "wall"}
    data = {
        "scene": scene_name,
...
def process_scene(
    ...
    image_root = scene_dir / resolve_image_subdir(data_root)
