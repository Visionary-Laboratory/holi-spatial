...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background, 
    ...
    if volume is not None:
        ...
        for idx, view in enumerate(tqdm(views, desc="TSDF Fusion progress")):
            ...
            pose = np.identity(4)
            pose[:3, :3] = view.R.transpose(-1, -2).cpu().numpy() if torch.is_tensor(view.R) else np.asarray(view.R).T
            pose[:3, 3] = view.T.cpu().numpy() if torch.is_tensor(view.T) else np.asarray(view.T).flatten()[:3]
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool,
    ...
    with torch.no_grad():
        ...
        # TSDF 使用 CPU 版本（当前 Open3D CUDA 构建未暴露 TSDFVoxelGrid）
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_depth", default=8.0, type=float)
