...
#   1) 指定场景：-s scene0305_01
#   2) 随机抽样：-n 5
#
# 示例：
#   bash run_random_scenes_aabb_scannet.sh -s scene0305_01
#   bash run_random_scenes_aabb_scannet.sh -s scene0305_01 --exclude-ids 123 456
#   bash run_random_scenes_aabb_scannet.sh -n 5 --seed 42
...
OUTPUT_DIR="${SCRIPT_DIR}/bbox_overlay_test_outputs_aabb_scannet_0221"
VIDEO_DIR="${OUTPUT_DIR}/videos"
MODEL_PATH="/home/liuyifei/code/posevlm/pgsr_result_scannetv2"
FPS=10
SCANNET_ROOT="/home/liuyifei/code/ScanNet"

print_help() {
  cat <<'EOF'
用法:
  bash run_random_scenes_aabb_scannet.sh -s <场景名> [<场景名> ...] [可选参数]
  bash run_random_scenes_aabb_scannet.sh -n <数量> [可选参数]

参数（二选一）:
  -s, --scene <名称>      指定场景名，可多个。如: -s scene0305_01
...
可选参数:
      ...
      --output-dir <path> 渲染图片输出目录
      --video-dir <path>  视频输出目录（默认: <output-dir>/videos）
      --model-path <path> 3DGS 模型目录
      --fps <int>         输出视频帧率（默认: 6）
      --scannet-root <path> ScanNet 仓库目录（SensReader 用，默认: /home/liuyifei/code/ScanNet）
  -h, --help              显示帮助
EOF
}
...
for scene_json in "${selected_jsons[@]}"; do
  ...
  sens_file="${SCANNET_ROOT}/scannetv2/scans/${scene_name}/${scene_name}.sens"
  if [[ -f "${sens_file}" ]]; then
    echo "    [SensReader] 运行 ${scene_name} 的 sens 导出..."
    (cd "${SCANNET_ROOT}" && python SensReader/python/reader.py \
      --filename "scannetv2/scans/${scene_name}/${scene_name}.sens" \
      --output_path "scannetv2/scans/${scene_name}/" \
      --export_poses \
      --export_color_images \
      --export_intrinsics)
    sens_exit=$?
    if [[ "${sens_exit}" -ne 0 ]]; then
      echo "    [失败] SensReader 退出码: ${sens_exit}"
      fail_count=$((fail_count + 1))
      continue
    fi
  else
    echo "    [警告] 未找到 .sens 文件: ${sens_file}，跳过 SensReader"
  fi
  ...
  python3 "${SCRIPT_DIR}/json2bbox_images_depth_scannet.py" "${scene_json}" \
    ...
    --downscale 1 \
    --thickness 3 \
    --exclude-labels tile,tiles,shelf,floor,ceiling,drawe,can,lamp,wall,stool \
    "${exclude_args[@]}"
  render_exit=$?
