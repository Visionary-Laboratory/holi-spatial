...
def verify_label_with_vlm(
    ...
    """使用 vLLM 服务验证标签是否正确。"""
    prompt = (
        "You are a visual region label verifier for a single highlighted functional area (activity region).\n"
        "You will be given three images: (1) the original image, (2) an overlay image where the target region is highlighted with a colored mask, and (3) a zoomed-in pair showing the original region and the highlighted region.\n"
        f'The user-provided region label to verify is: "{category}".\n\n'

        "Task:\n"
        "Decide whether the given label correctly describes the FUNCTIONAL REGION indicated by the HIGHLIGHTED area.\n"
        "If it is correct, ACCEPT. If it is incorrect (wrong region type), REJECT and provide the correct region label you believe fits best.\n\n"

        "Guidelines (consistency-and-rewrite mindset):\n"
        "1) Focus ONLY on the highlighted area. Ignore other parts of the scene outside the mask.\n"
        "2) Identify the most likely functional use of the highlighted area (e.g., sleep area, study area, cooking area, dining area, bathroom area, hallway, entry area, storage area, living area).\n"
        "3) The label refers to an ACTIVITY REGION, not an object. Use objects only as evidence for the region function (e.g., bed -> sleep area; desk -> study area; stove/sink -> cooking area).\n"
        "4) If the highlighted mask covers only part of the functional area, infer the full region function from visible cues.\n"
        "5) If the highlighted area contains multiple functions, choose the dominant/primary function.\n"
        "6) If the highlighted area is ambiguous, too small, or does not correspond to a recognizable functional region, REJECT and set predicted_label to \"unknown\".\n"
        "7) Prefer canonical, general region labels over overly specific ones. Avoid style-based or decorative labels.\n"
        "8) Consistency: accept reasonable equivalents only if they map to the SAME canonical region label. If you use a canonical vocabulary, always output the canonical label.\n\n"

        "Output format (VERY IMPORTANT):\n"
        "Return ONLY a JSON object on a single line with these keys:\n"
        "  - decision: \"ACCEPT\" or \"REJECT\"\n"
        "  - predicted_label: a short lowercase English region label (e.g., \"sleep area\", \"study area\", \"cooking area\").\n"
        "Rules:\n"
        "  - If decision is ACCEPT, predicted_label should be exactly the provided label (category) or its closest canonical normalized form.\n"
        "  - If decision is REJECT, predicted_label must be your best guess of the correct canonical region label, or \"unknown\" if unclear.\n"
    ...
    for label, label_items in label_groups.items():
        ...
        for inst in final_instances:
            ...
            if instance_max_points > 0 and pts.shape[0] > instance_max_points:
                ...
                pts = pts[idx]
            inst["obb_transform"], inst["obb_extents"] = compute_obb(pts)
            inst["points"] = [pts]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--erode-pixels", type=int, default=25, help="mask 形态学腐蚀像素，过滤边缘")
    parser.add_argument(
        "--instance-max-points",
        type=int,
        default=100000,
