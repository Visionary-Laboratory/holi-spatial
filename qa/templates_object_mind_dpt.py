...
def build_object_dpt_entry(
    ...
    *,
    ...
    question = textwrap.dedent(
        ...
        Consider the camera transformation from image A to image B.
...
def build_object_orient_entry(
    ...
    *,
    ...
    """
    基于 bbox json 与 mask 路径构造“给定 A 视角的 mask，问物体在 B 视角的方位”QA。
    ...
    question = textwrap.dedent(
        ...
        In image A, the white mask highlights the target object "{label}".
        Consider the camera transformation from image A to image B.
        Use the camera center of image B as the reference.
        Determine ONLY the relative direction of the target object center with respect to the image B camera center:
        - horizontal: left or right
        - vertical: up or down
        - depth: front or back (in front of / behind the camera)

        Output EXACTLY in the following format and do not output anything else:
        """
        """
        <answer>{left|right}, {up|down}, {front|back}</answer>
        """
    ).strip()

    answer = ""
    if target['center_cam_b'][0] > 0:
        answer += "right, "
    else:
        answer += "left, "
    if target['center_cam_b'][1] > 0:
        answer += "up, "
    else:
        answer += "down, "
    if target['center_cam_b'][2] > 0:
        answer += "back"
    else:
        answer += "front"
    ...
    return {
        ...
        "covisibility": cov,
        "threshold": threshold,
        "question_type": "object_orient",
        "sub_question_type": "bbox_center_orient",
        "question": question,
        "answer": answer,
        "selected_ins_id": target["ins_id"],
        "objects": shared_instances,
