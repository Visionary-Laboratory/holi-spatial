...
if TYPE_CHECKING:
    # Only for type hints to avoid runtime import cycles.
...
def build_cam_rotation_entry(
    ...
    *,
    ...
    def axis_dir_word(axis_name: str, val_deg: float) -> str:
        if axis_name == "yaw":
            return "turn left" if val_deg > 0 else "turn right"
        if axis_name == "pitch":
            return "tilt up" if val_deg > 0 else "tilt down"
        if axis_name == "roll":
            return "counter-clockwise roll" if val_deg > 0 else "clockwise roll"
    ...
    def axis_two_options(axis_name: str) -> Tuple[str, str]:
        ...
        if axis_name == "roll":
            return ("counter-clockwise roll", "clockwise roll")
        raise ValueError(f"Unknown axis: {axis_name}")
    ...
    rotation_details = {
        ...
        "yaw_deg_left_right": float(yaw_deg),     # + => left
        "pitch_deg_up_down": float(pitch_deg),    # + => up
        "roll_deg_ccw_cw": float(roll_deg),       # + => CCW
    }
