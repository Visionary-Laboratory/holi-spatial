...
def _resize_mask_to_square(mask_u8: np.ndarray, target_size: int) -> torch.Tensor:
    """
    Sam3Tracker 的 input_masks 需要 (B, image_size, image_size)，image_size 默认 1008。
    这里把 0/255 mask resize 到 target_size，并转 float32 0/1。
    """
    m = (mask_u8 > 0).astype(np.uint8)
    m_rs = cv2.resize(m, (target_size, target_size), interpolation=cv2.INTER_NEAREST)
    return torch.from_numpy(m_rs.astype(np.float32))  # (S, S)


def get_mask_from_sam3_tracker(
    original_mask: np.ndarray,      # sparse 0/255
    coarse_mask: np.ndarray,        # expanded 0/255
    label_to_query: str,            # 你原先传 label；tracker 这里不需要 text，但保留接口
    image_path: Optional[Path],
    trk_model: Optional["Sam3TrackerModel"],
    trk_processor: Optional["Sam3TrackerProcessor"],
    max_positive_points: int = 5,
    max_negative_points: int = 3,
    mask_selection: str = "best",   # "best" or "merge"
) -> np.ndarray:
    """
    使用 HF Sam3Tracker 做“粗 mask + 点”精修。
    - 用 coarse_mask 作为 input_masks（可选但很有用）
    - 用 sparse/coarse 采样正负点作为 click prompts
    """

    coarse_mask = original_mask

    if trk_model is None or trk_processor is None:
        logging.warning("Sam3Tracker 未加载，直接返回 coarse_mask")
        return coarse_mask

    if image_path is None or (not isinstance(image_path, Path)) or (not image_path.exists()):
        logging.warning("图像不存在，直接返回 coarse_mask")
        return coarse_mask

    # 没有有效区域直接返回
    if np.sum(coarse_mask > 0) == 0:
        return coarse_mask
    ...
    # 采样点（你原来的采样函数可以继续用）
    pos_pts = _sample_points_from_mask(original_mask, max_positive_points)
    neg_pts = _sample_negative_points(coarse_mask, max_negative_points)
    ...
    all_pts = pos_pts + neg_pts
    pts_xy = [[float(x), float(y)] for (x, y) in all_pts]

    input_points = [[pts_xy]]  # ✅ 4 层： [image][object][point][xy]
    input_labels = [[[1] * len(pos_pts) + [0] * len(neg_pts)]]  # ✅ 3 层：[image][object][point]


    # 1) 先算 image_embeddings（这样 refine 更省）
    inputs_img = trk_processor(images=raw_image, return_tensors="pt").to(trk_model.device)

    with torch.no_grad():
        image_embeddings = trk_model.get_image_embeddings(**inputs_img)

    # 2) 准备 prompts（点 + mask）
    # Sam3Tracker 的 image_size 默认 1008（配置里就是这个量级），这里用模型 config 取更稳
    try:
        target_size = int(trk_model.config.prompt_encoder_config.image_size)
    except Exception:
        target_size = 288
    
    # target_size = 288

    mask_in = _resize_mask_to_square(coarse_mask, target_size).to(trk_model.device)  # (S,S)
    mask_in = mask_in.unsqueeze(0).unsqueeze(1)  # ✅ (B=1, C=1, S, S)
    print("mask_in shape:", mask_in.shape, mask_in.dtype, mask_in.device)
    
    inputs_prompt = trk_processor(
        input_points=input_points,
        input_labels=input_labels,
        original_sizes=inputs_img["original_sizes"],  # 让 post_process 回到原图大小
        return_tensors="pt",
    ).to(trk_model.device)

    multimask_output = True  # 先输出多候选，再按策略选
    with torch.no_grad():
        outputs = trk_model(
            **inputs_prompt,
            input_masks=mask_in,
    ...
    print("outputs.pred_masks:", outputs.pred_masks.shape, outputs.pred_masks.dtype, outputs.pred_masks.device)
    # post-process 回原图尺寸
    masks = trk_processor.post_process_masks(outputs.pred_masks.cpu(), inputs_img["original_sizes"])[0]
    # masks: shape 通常是 (num_objects=1, num_masks=3, H, W) :contentReference[oaicite:2]{index=2}

    if masks is None or masks.numel() == 0:
        return coarse_mask

    # 选择 best 或 merge
    mask_selection = (mask_selection or "best").lower()

    if mask_selection == "merge":
        fused = (masks[0] > 0).any(dim=0).numpy().astype(np.uint8) * 255
        return fused
    else:
        # best: 按 iou_scores 选最好那个 :contentReference[oaicite:3]{index=3}
        best_idx = 0
        if hasattr(outputs, "iou_scores") and outputs.iou_scores is not None:
            # iou_scores 常见 shape: (B, num_objects, num_masks)
            scores = outputs.iou_scores.detach().cpu().float().reshape(-1)
            if scores.numel() > 0:
                best_idx = int(torch.argmax(scores).item() % masks.shape[1])

        best = (masks[0, best_idx] > 0).numpy().astype(np.uint8) * 255
        return best
...
def process_scene(
    ...
    # 3. 从 rrd 文件读取每个 label 的点云（保持实例级别，不合并）
    try:
    ...
    for image_name, label_masks in tqdm(gt_masks.items(), desc="处理图像"):
        ...
        for label, gt_mask in label_masks.items():
            ...
            for inst_idx, label_points in enumerate(instance_points_list):
                ...
                pred_mask = get_mask_from_sam3_tracker(
                    original_mask=pred_mask_sparse,
                    coarse_mask=pred_mask_expanded,
                    label_to_query=label,
                    image_path=image_path if image_path else None,
                    trk_model=sam3_model,
                    trk_processor=sam3_processor,
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("gyn_test_images/2d_iou_outputs_gyn_cmp_sam3_tracker"), help="输出目录")
    ...
    parser.add_argument("--mask-selection", type=str, choices=["merge", "best"], default="best", help="SAM3 多候选 mask 的选择方式：merge=加权融合，best=取最高分")
    ...
    try:
        ...
        model_id = "/mnt/shared-storage-user/intern7shared/liuyifei/code/sam3_trancker_env/sam3"
