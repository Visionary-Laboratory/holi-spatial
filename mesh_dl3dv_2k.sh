...
# 批量处理 DL3DV 2K 数据集：对 processed_dl3dv_ours/2K 下所有场景跑 render.py + mesh2mask.py
# 若场景已有 mesh/tsdf_fusion_post.ply 则跳过；使用多卡并行，每张卡处理一个场景

echo "开始批量处理 DL3DV 2K 下的所有场景..."

DATA_ROOT="processed_dl3dv_ours/2K"
OUTPUT_ROOT="pgsr_DL3DV_all/2K"
...
for scene_dir in "${DATA_ROOT}"/*/; do
    ...
    mesh_file="${OUTPUT_ROOT}/${scene}/mesh/tsdf_fusion_post.ply"
