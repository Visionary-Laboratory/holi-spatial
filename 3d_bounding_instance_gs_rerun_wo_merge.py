    ...
    for label, label_items in label_groups.items():
        ...
        for item in tqdm(label_items, desc=f"Processing {label}"):
            ...
            for idx in indices_to_check:
                ...
                # 检查新 instance 是否与该实例共享同一个 frame
                # 如果共享同一个 frame，不能 merge（必须是两个独立的实例）
                conflict_with_new = any(
                    share_same_frame(img, new_img)
                    for img in inst["images"]
                    for new_img in new_inst["images"]
                )
                
                if conflict_with_new:
                    # 共享同一个 frame，不能 merge
                    continue
                
                # 检查该实例是否与 merge_indices 中已有的实例共享同一个 frame
                # 如果共享，不能 merge（会导致来自同一图片的 mask 被 merge）
                can_merge_with_others = True
                for already_selected_idx in merge_indices:
                    already_inst = instances[already_selected_idx]
                    if any(share_same_frame(img1, img2) 
                           for img1 in already_inst["images"] 
                           for img2 in inst["images"]):
                        can_merge_with_others = False
                        break
                
                # 只有不冲突且可以与其他已选实例 merge 时，才添加到 merge_indices
