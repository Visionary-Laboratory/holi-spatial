...
def _depth_visible_mask(
    ...
    visible[valid_scene] = z[valid_scene] <= (scene_z[valid_scene] + float(depth_eps))
    return visible
...
def _center_depth_visible(
    ...
    if not np.isfinite(scene_z) or scene_z <= 0.0:
        return True
    return center_z <= scene_z + float(depth_eps)
...
def _draw_depth_aware_edge(
    ...
    if fast_mode:
        ...
        occ = np.isfinite(scene_z) & (scene_z > 0.0) & (z > (scene_z + float(depth_eps)))
        valid &= ~occ
    ...
    for t in ts:
        ...
        if np.isfinite(scene_z) and scene_z > 0.0 and z > scene_z + float(depth_eps):
            visible_pts.append((-1, -1))
            continue
        ...
        for zc, box, corners_w, corners_c, camera_inside in prepared:
            ...
            for f in _FACES:
                ...
                for i in range(1, clipped_cam.shape[0] - 1):
                    ...
                    if yy.size == 0:
                        continue
                    visible = _depth_visible_mask(yy, xx, tri_z, scene_depth, depth_eps)
        ...
        for zc, box, corners_w, corners_c, camera_inside in prepared:
            ...
            if show_labels:
                ...
                if zc2[0] > _NEAR_PLANE:
                    if not _center_depth_visible(uv_c[0], float(zc2[0]), scene_depth, depth_eps, img_w, img_h):
                        continue
